...
import { useAppointments } from "../VISITS/Hooks/useAppointments.jsx";
import DayBox from './DayBox.jsx';
import { getMonthYearDetails } from '../VISITS/Hooks/monthYear.jsx';
...
const Appointments = () => {
  ...
  const isSunday = (date) => {
   ...
   <div className='flex gap-5 justify-center mb-5'>
      ...
      <div className='grid  md:grid-cols-7  gap-2 m-2'>
    
{[...Array(monthYear.lastDate)].map((_, i) => {   
const date = i + 1;
const firstDayOfMonth = dayjs(`${monthYear.year}-${monthYear.month}-1`).date();
 const isFirstDayOfMonth = date === firstDayOfMonth;

 const monthDetails = getMonthYearDetails(dayjs(`${monthYear.year}-${monthYear.month}-01`));
 const dayObject = monthDetails.days[i]; // Access the day object
const dayName = dayObject.dayName

 const calculatedDivColumn = (
  (i + 1 + (monthYear.firstDOW === 0 ? 6 : monthYear.firstDOW) - 1) % 7
);
       
    console.log(`Day: ${i + 1}, divColumn: ${calculatedDivColumn} (DOW: ${monthYear.firstDOW})`);
    return (
        <DayBox      
                   key={i}
                        date={i + 1}           
            ...
            monthYear={monthYear}
            calculatedDivColumn={calculatedDivColumn}
            isFirstDayOfMonth={isFirstDayOfMonth}
            dayName={dayObject.dayName} // Pass the day name as a prop         
        >
